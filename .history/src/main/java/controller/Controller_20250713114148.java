...
private boolean corrispondeRicerca(ToDo todo, String termine) {
    ...
    // Cerca nelle attività della checklist
    if (todo.getChecklist() != null && todo.getChecklist().getAttivita() != null) {
        for (Attivita attivita : todo.getChecklist().getAttivita()) {
            if (attivita.getNome() != null && attivita.getNome().toLowerCase().contains(termine)) {
                return true;
            }
        }
        
        // Cerca nel nome della checklist
        if (todo.getChecklist().getNomeChecklist() != null && 
            todo.getChecklist().getNomeChecklist().toLowerCase().contains(termine)) {
            return true;
        }
    }
...
private void raccogliTuttiToDo(ArrayList<ToDo> risultati) {
    ...
    private void mostraMain() {
        ...
        mainView.getMostraInScadenza().addActionListener(_->
                mainView.getBaSca().setVisible(true));
...
if (attivitaDaSalvareNuovo != null && !attivitaDaSalvareNuovo.isEmpty()) {
    ...
    private void salvaTodoNelDatabase(ToDo todo, JDialog dialog, boolean isUpdate) {
        try {
            if (isUpdate) {
                toDoDAO.modificaToDo(utenteAttuale.getEmail(), todo);
            } else {
                toDoDAO.creaToDo(utenteAttuale.getEmail(), todo);
            }
