...
public class Controller {
    ...
    private void mostraMain() {
        ...
        rimuoviTuttiActionListener(mainView.getAggiungiToDo());
        rimuoviTuttiActionListener(mainView.getMostraCompletati());
        rimuoviTuttiActionListener(mainView.getEsci());
        rimuoviTuttiActionListener(mainView.getMostraInScadenza());
        rimuoviTuttiActionListener(mainView.getButtonAzzera());
        ...
        // Rimuovi KeyListener esistenti prima di aggiungerne uno nuovo
    for (KeyListener kl : mainView.getCampoRicerca().getKeyListeners()) {
        mainView.getCampoRicerca().removeKeyListener(kl);
    }
    ...
    private List<ToDo> ordinaToDoList(List<ToDo> toDoList, Ordinamento ordinamento) {
        switch (ordinamento) {
            case AZ:
    ...
    /**
     * Funzione estratta da generaActionListenerSalvaModifica per sopperire al code smell cognitivo
     */
    private void controlloArrayAttivita(ArrayList<JTextField> attivitaFieldsNuovo,AtomicReference<JTextField> titoloCheckListRefNuovo,ToDo todo){
        ArrayList<Attivita> attivitaDaSalvareNuovo = estraiAttivitaDaFields(attivitaFieldsNuovo);
        if (!attivitaDaSalvareNuovo.isEmpty() && titoloCheckListRefNuovo.get() != null) {
        ...
        } else if ( attivitaDaSalvareNuovo.isEmpty()) {
    ...
    private ActionListener generaActionListnerSalvaModifica(CreaToDo modificaTodoDialog, ToDo todo,
        ...
        return _ -> {
            ...
            controlloArrayAttivita(attivitaFieldsNuovo,titoloCheckListRefNuovo,todo);
    ...
    public JButton generaEliminaButton(Color coloreTesto, ToDo todo, Condivisione condivisione) {
        ...
        eliminaButton.addActionListener(e -> {
            ...
            if (condivisione != null) {
                ...
                if (Boolean.TRUE.equals(controlloRimozioneToDoList(todo))) {
                    try {
    ...
    /** Funzione estratta da @link{generaEliminaButton()} per sopperire al code smell cognitivo
     * @param todo da controllare
     */
    private Boolean controlloRimozioneToDoList(ToDo todo){
        if (utenteAttuale.getTempoLibero().getToDoList().remove(todo))
            return true;
        else if (utenteAttuale.getLavoro().getToDoList().remove(todo))
            return true;
        else return utenteAttuale.getUniversita().getToDoList().remove(todo);
    }
    ...
    private void creazioneCondividiDialog(JButton condiviButton, ToDo todo) {
        ...
        condiviButton.addActionListener(e -> {
            ...
            listaUtenti.getCondivButton().addActionListener(ev -> {
                List<String> selezionati = getUtentiSelezionati(utentiCb);
    ...
    private void gestisciPassDime(@NotNull LogIn loginView) {
        loginView.getPassDime().addActionListener(e -> {
            ...
            rimuoviTuttiActionListener(resetDialog.getBottoneReset());
     ...
     * @param todo
     */
    public void generaUtentiCondivisibili(ListaUtenti listaUtenti, List<Utente> utenti, ToDo todo,List<JCheckBox> utentiCb) {
     ...
     */
    public List<String> getUtentiSelezionati(List<JCheckBox> utentiCb) {
