    ...
    public Bacheca caricaBacheca(String emailUtente, Titolo titolo) {
        ...
        try {
            PreparedStatement caricaPS = connection.prepareStatement(
                    "SELECT * FROM \"Bacheca\" NATURAL JOIN \"ToDo\" WHERE \"emailUtente\" = ? AND \"titolo\" = ?::titolo                    @Override
                    public Bacheca caricaBacheca(String emailUtente, Titolo titolo) {
                        Bacheca b = null;
                        try {
                            PreparedStatement caricaPS = connection.prepareStatement(
                                    "SELECT * FROM \"Bacheca\" NATURAL JOIN \"ToDo\" WHERE \"emailUtente\" = ? AND \"titolo\" = ?::titolo");
                            caricaPS.setString(1, emailUtente);
                            caricaPS.setObject(2, titolo.name(), java.sql.Types.OTHER);
                    
                            ResultSet rs = caricaPS.executeQuery();
                    
                            if (rs.next()) {
                                Titolo titoloBacheca = Titolo.valueOf(rs.getString("titolo"));
