...
/**
 * Implementazione concreta dell'interfaccia {@link dao.UtenteDAO} per
 * PostgreSQL.
...
public class UtenteImplementazionePostgresDAO implements dao.UtenteDAO {
    ...
    /**
     * Costruttore che ottiene una connessione al database tramite
     * {@link DBConnessione}.
     */
    ...
    public void registraUtente(String email, String password) {
        ...
        try (PreparedStatement registratiPS = connection.prepareStatement(sql)) {
    ...
    public boolean loginValido(String email, String password) {
        ...
        try (PreparedStatement loginValidoPS = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = loginValidoPS.executeQuery()) {
                return rs.next();
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public Utente trovaUtenteDaMail(String email) {
        String sql = "SELECT \"email\",\"password\" FROM \"Utente\" WHERE \"email\" = ?";

        try (PreparedStatement trovaUtentePS = connection.prepareStatement(sql)) {
            trovaUtentePS.setString(1, email);

            try (ResultSet rs = trovaUtentePS.executeQuery()) {
                if (rs.next()) {
                    Utente u = new Utente();
    ...
    public boolean aggiornaPassword(String email, String nuovaPassword) {
        ...
        try (PreparedStatement aggiornaPasswordPS = connection.prepareStatement(sql)) {
