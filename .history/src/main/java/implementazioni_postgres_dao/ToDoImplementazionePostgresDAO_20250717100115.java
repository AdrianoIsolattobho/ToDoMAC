...
public class ToDoImplementazionePostgresDAO implements dao.ToDoDAO {
    ...
    public void creaToDo(String emailUtente, ToDo toDo, Bacheca bacheca) {
        try {
            ...
            // VERIFICA SE LA BACHECA ESISTE GIÀ - CORREGGI QUESTA PARTE
            PreparedStatement checkBachecaPS = connection.prepareStatement(
                    "SELECT COUNT(DISTINCT \"emailUtente\", \"titolo\") as count FROM \"Bacheca\" " +
                            "WHERE \"emailUtente\" = ? AND \"titolo\" = CAST(? AS titolo)");
            checkBachecaPS.setString(1, emailUtente);
            checkBachecaPS.setString(2, bacheca.getTitolo().toString());

            ResultSet checkRs = checkBachecaPS.executeQuery();
            boolean bachecaEsiste = false;
            if (checkRs.next()) {
                bachecaEsiste = checkRs.getInt("count") > 0;
                System.out.println("Controllo bacheca - Count: " + checkRs.getInt("count") + ", Esiste: " + bachecaEsiste);
            }
            checkRs.close();
            checkBachecaPS.close();

            // INSERISCI LA BACHECA SOLO SE NON ESISTE
            if (!bachecaEsiste) {
                System.out.println("Creando nuova bacheca: " + bacheca.getTitolo());
                ...
                System.out.println("Bacheca già esistente: " + bacheca.getTitolo());
                // Associa il ToDo alla bacheca esistente inserendo un nuovo record
                PreparedStatement associaBachecaPS = connection.prepareStatement(
                        "INSERT INTO \"Bacheca\" (\"titolo\",\"emailUtente\", \"descrizione\", \"IdToDo\", \"ordinamento\") " +
                                "SELECT \"titolo\", \"emailUtente\", \"descrizione\", ?, \"ordinamento\" " +
                                "FROM \"Bacheca\" " +
                                "WHERE \"emailUtente\" = ? AND \"titolo\" = CAST(? AS titolo) " +
                                "LIMIT 1");
                associaBachecaPS.setInt(1, nuovoIdToDo);
                associaBachecaPS.setString(2, emailUtente);
                associaBachecaPS.setString(3, bacheca.getTitolo().toString());
                associaBachecaPS.executeUpdate();
                associaBachecaPS.close();
            }
